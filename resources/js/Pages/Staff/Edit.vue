  ...
  <DashboardLayout>
    ...
    <div class="d-flex justify-content-between align-items-start mb-4">
      <div>
        ...
        <p class="mb-0 text-muted">Update {{ staff.full_name }}'s profile information</p>
      </div>
      <div class="d-flex gap-2">
        <Link :href="route('staff.show', staff.id)" class="btn btn-outline-info">
          View Profile
        </Link>
      ...
      <div class="card border-0 shadow-sm">
        <div class="card-body">
          ...
          <div v-show="currentStep === 0" class="step-content">
            ...
            <div class="row g-3">
              <!-- Row 1: First Name, Middle Name, Last Name, Profile Photo -->
              <div class="col-md-3">
              ...
              <!-- Row 2: Date of Birth, Gender, Country, Hire Date -->
              <div class="col-md-3">
          ...
          <div v-show="currentStep === 1" class="step-content">
            ...
            <div class="row g-3">
              <!-- Row 1: Department, Position, Status, Employment Type -->
              <div class="col-md-3">
              ...
              <!-- Row 2: Work Location, Current Base, Contract Start Date, Contract End Date -->
              <div class="col-md-3">
              ...
              <div class="col-md-3">
                ...
                <input
                  ...
                  id="current_base"
                >
                <small class="form-text text-muted">For crew members</small>
              </div>

              <div class="col-md-3" v-show="showContractDates">
              ...
              <!-- Row 3: Supervisor, Salary -->
              <div class="col-md-3">
                <label for="supervisor_name" class="form-label">Supervisor / Manager</label>
                <input
          ...
          <div v-show="currentStep === 2" class="step-content">
            <h5 class="card-title mb-4">Documents & Education</h5>
            <div class="clearfix"></div>

            <div class="row g-3">
              <!-- Row 1: ID Type, ID Number, ID File, Highest Qualification -->
              <div class="col-md-3">
              ...
              <!-- Row 2: Qualification Name, Institution, Graduation Year, Additional Info -->
              <div class="col-md-3">
                ...
                <input
                  ...
                  placeholder="e.g. Bachelor of Science in Aviation"
                >
              ...
              <div class="col-md-3">
                ...
                <textarea
                  ...
                  class="form-control"
                  :class="{ 'is-invalid': errors.additional_info }"
                  id="additional_info"
                  rows="1"
                  placeholder="Skills, certifications, etc."
                ></textarea>
                <div v-if="errors.additional_info" class="invalid-feedback">
...
// Form data pre-populated with staff data
...
const formatDateForInput = (date) => {
  ...
  // If date is a Date object or other format, convert to YYYY-MM-DD
  const d = new Date(date)
  if (isNaN(d.getTime())) return ''
  return d.toISOString().split('T')[0]
}

// Helper function to safely get string value from field (could be string or relationship object)
...
const form = useForm({
  ...
  profile_photo: props.staff.profile_photo || null,
  date_of_birth: formatDateForInput(props.staff.date_of_birth),
  gender_id: props.staff.gender_id || '',
  country_id: props.staff.country_id || '',
  hire_date: formatDateForInput(props.staff.hire_date),

  // Employment Information
  contract_start_date: formatDateForInput(props.staff.contract_start_date),
  contract_end_date: formatDateForInput(props.staff.contract_end_date),
  department_id: props.staff.department_id || '',
  position_id: props.staff.position_id || '',
  status_id: props.staff.status_id || '',
  employment_type_id: props.staff.employment_type_id || '',
  current_base: props.staff.current_base || '',
  work_location_id: props.staff.work_location_id || '',
  supervisor_name: props.staff.supervisor_name || '',
  salary: props.staff.salary || '',

  // Contact Information
  email: props.staff.email || '',
  phone_primary: props.staff.phone_primary || '',
  phone_secondary: props.staff.phone_secondary || '',
  address_line_1: props.staff.address_line_1 || '',
  address_line_2: props.staff.address_line_2 || '',
  city: props.staff.city || '',
  state_region: props.staff.state_region || '',
  postal_code: props.staff.postal_code || '',
  country: getStringValue(props.staff.country),
  notes: props.staff.notes || '',

  // Next of Kin Information
  next_of_kin_name: props.staff.next_of_kin_name || '',
  next_of_kin_phone: props.staff.next_of_kin_phone || '',
  next_of_kin_email: props.staff.next_of_kin_email || '',
  next_of_kin_relationship: props.staff.next_of_kin_relationship || '',

  // Documents & Education
  identification_type: getStringValue(props.staff.identification_type), // Keep for backward compatibility
  identification_type_id: props.staff.identification_type_id || '',
  identification_number: props.staff.identification_number || '',
  identification_file: null,
  highest_qualification_id: props.staff.highest_qualification_id || '',
  qualification_name: props.staff.qualification_name || '',
  institution_name: props.staff.institution_name || '',
  graduation_year: props.staff.graduation_year || '',
  additional_info: props.staff.additional_info || ''
})
...
const canProceed = computed(() => {
  switch (currentStep.value) {
    case 0: // Basic Information (includes contact details and next of kin - all optional except core fields)
      // More lenient validation for existing records that might be missing new fields
      const step0Valid = !!(form.first_name && form.last_name && form.date_of_birth)
      return step0Valid
    case 1: // Employment Information
      // More lenient validation for existing records
      const step1Valid = !!(form.department_id && form.position_id && form.status_id)
      return step1Valid
...
const filterPositions = () => {
  // Don't reset position if it's still valid for the new department
  const isPositionValid = filteredPositions.value.some(pos => pos.id == form.position_id)
  if (!isPositionValid) {
    form.position_id = ''
  }
}

// Contract dates visibility
const showContractDates = computed(() => {
  if (!form.employment_type_id) return false
  const selectedType = props.employmentTypes.find(type => type.id == form.employment_type_id)
  return selectedType ? selectedType.requires_contract_dates : false
})
...
// Update contract fields when employment type changes
const updateContractFields = () => {
  if (!showContractDates.value) {
...
const submitForm = () => {
  form.put(route('staff.update', props.staff.id), {
    ...
    onError: (errors) => {
      ...
      if (errorFields.some(field => ['first_name', 'last_name', 'date_of_birth', 'gender_id', 'country_id', 'hire_date', 'profile_photo', 'email', 'phone_primary', 'phone_secondary', 'address_line_1', 'address_line_2', 'city', 'state_region', 'postal_code', 'country', 'notes', 'next_of_kin_name', 'next_of_kin_phone', 'next_of_kin_email', 'next_of_kin_relationship'].includes(field))) {
        currentStep.value = 0
      } else if (errorFields.some(field => ['department_id', 'position_id', 'status_id', 'employment_type_id', 'work_location_id', 'contract_start_date', 'contract_end_date', 'supervisor_name', 'salary'].includes(field))) {
        currentStep.value = 1
      } else if (errorFields.some(field => ['identification_type', 'identification_number', 'identification_file', 'highest_qualification_id', 'qualification_name', 'institution_name', 'graduation_year', 'additional_info'].includes(field))) {
        currentStep.value = 2
      }
      scrollToTop()
    }
