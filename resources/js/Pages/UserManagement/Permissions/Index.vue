    ...
    <div class="page-title-wrapper">
      <div class="page-title-heading">
        ...
        <div>
          ...
          <div class="page-title-subheading">Manage system permissions and access controls</div>
        </div>
      </div>
    ...
    <div class="main-card mb-3 card">
      ...
      <div class="card-body">
        ...
        <div class="alert alert-info" v-if="true">
          <strong>Debug:</strong>
          <br>Permissions object: {{ permissions ? 'exists' : 'null' }}
          <br>Permissions.data: {{ permissions?.data ? 'exists' : 'null' }}
          <br>Data length: {{ permissions?.data?.length || 0 }}
          <br>Data type: {{ Array.isArray(permissions?.data) ? 'array' : typeof permissions?.data }}
        </div>
            ...
            <table class="table table-hover">
              ...
              <tbody>
                <tr v-for="permission in (permissions?.data || [])" :key="permission.id">
                  ...
                  <td>
                    <span class="badge badge-primary">{{ getPermissionCategory(permission.name) }}</span>
                  </td>
                  <td>
                  ...
                  <td>
                    ...
                    <div class="mt-1" v-if="permission.roles?.length > 0">
                      <small class="text-muted">
                        {{ permission.roles.slice(0, 2).map(r => r.name).join(', ') }}
                        <span v-if="permission.roles.length > 2">
...
export default {
  ...
  data() {
    return {
      viewMode: 'table', // 'table' or 'grouped'
  ...
  mounted() {
    console.log('Permissions Index mounted - permissions data:', this.permissions)
  },
  ...
  methods: {
    ...
    deletePermission() {
      ...
      try {
        const url = route('permissions.destroy', this.permissionToDelete.id)
        router.delete(url, {
          onSuccess: () => {
            this.showDeleteModal = false
